...
class stuff(object):
     ...
     def update(self, max_iter, check_iter, min_iter, tol):
        ...
        np.savetxt("supreme_wfc_mean_iter_0.txt"       , self.lnX ,fmt='%.12f')
        np.savetxt("supreme_flux_iter_0.txt"       , self.F ,fmt='%.12f')
        np.savetxt("supreme_wfc_background_iter_0.txt" , self.B ,fmt='%.12f')
        ...
        for i in range(max_iter):
            ...
            np.savetxt("supreme_wfc_mean_iter_%d.txt"%(i+1)       , self.lnX ,fmt='%.12f')
            np.savetxt("supreme_wfc_flux_iter_%d.txt"%(i+1)       , self.F ,fmt='%.12f')
            np.savetxt("supreme_wfc_background_iter_%d.txt"%(i+1) , self.B ,fmt='%.12f')
            chi.append(obj)
