...
class stuff(object):
	...
	# save metadata
     ...
     def update_centroids(self, iternum):
        ...
        GG  = hfpy.File('masked_samplerx3%d.hdf5'%(iternum),'w')   
        Grp = GG.create_group("masked_samplerx3")
        for p in range(self.N):
    
          xp, yp = fit((self.dx[p], self.dy[p]), \
                       masked_data[p], self.masks[p], \
	...
	Pool = InterruptiblePool(Nthreads)
     ...
     def bfgs_lnX(self, num_funccalls):
        ...
        x = op.fmin_l_bfgs_b(self.func_grad_lnX_Nthreads, x0=self.lnX, fprime = None, \
                             ...
                             bounds = [(np.log(1e-5), 0.) for _ in self.lnX], m=10000, factr=1e7, pgtol=1e-01, epsilon=1e-01, maxfun=num_funccalls)
        gx = x[2]["grad"]
        print x
        print gx
     ...
     def update(self, max_iter, check_iter, min_iter, tol):
	    ...
	    print time.time() - a
            ...
            self.bfgs_lnX(500)
	    print time.time() - a
            np.savetxt("superb_wfc_mean_iter_%d_nfl.txt"%(i)       , self.lnX ,fmt='%.64f')
            np.savetxt("superb_wfc_flux_iter_%d_nfl.txt"%(i)       , self.F ,fmt='%.64f')
	    np.savetxt("superb_wfc_bkg_iter_%d_nfl.txt"%(i)        , self.B ,fmt='%.64f')
            """
            if (i==max_iter):
